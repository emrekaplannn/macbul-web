// app/api/feedback/match/route.ts
...
const log = (...a: any[]) => {
  if (DEBUG) console.debug("[FEEDBACK:match]", ...a);
};
...
export async function POST(req: Request) {
  ...
  const body = await req.text(); // MatchFeedbackUpsert JSON

  const url = `${base}/v1/match-feedback`;
